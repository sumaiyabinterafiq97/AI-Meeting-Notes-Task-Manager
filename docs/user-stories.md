...
| ID      | Priority | Story                                                                                                                                                            |
| ------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| MTG-01  | P0       | As a **team member**, I want to **create a meeting record** (title, date, attendees), so that **recordings and transcripts are organized**.                      |
| MTG-02  | P0       | As a **meeting creator**, I want to **paste a transcript or upload a text file**, so that **AI can process it without audio**.                                   |
| MTG-02b | P0       | As a **meeting creator**, I want to **upload or screen-record audio/video**, so that **I can capture the meeting without pasting text**.                         |
| MTG-02c | P0       | As a **meeting creator**, I want to **click Translate & Transcribe after upload**, so that **I control when Whisper runs and get an English transcript for AI**. |
...
| MTG-06  | P0       | As a **user**, I want to **see processing status** (draft, uploaded, translating, processing, ready, failed), so that **I know when AI output is ready**.        |
| MTG-07  | P1       | As a **user**, I want to **attach optional context** (agenda, project tags), so that **AI output is more relevant**.                                             |
| MTG-08  | P1       | As a **user**, I want to **replace a recording without auto-running AI**, so that **I can fix a bad capture before spending transcription quota**.               |

### Acceptance Criteria — MTG-02 (Paste / text transcript)

- User can paste text or upload `.txt`, `.md`, `.vtt`, `.srt` (≤ 5 MB)
- Minimum 100 characters required
- Transcript stored and AI job enqueued
- Meeting status changes to `PROCESSING` (skips `TRANSCRIBING`)
- User sees processing indicator on meeting detail page

### Acceptance Criteria — MTG-02b / MTG-02c (Recording capture)

- User can upload `.mp3` / `.m4a` / `.wav` / `.mp4` / `.webm` or use in-app screen/tab recorder
- Upload alone → meeting `DRAFT`, audio `PENDING`, **no** transcript / AI yet (`201`)
- **Translate & Transcribe** starts pipeline → `TRANSCRIBING` → `PROCESSING` → `READY`
- Default mode produces English transcript (Bengali+English → English)
- Replace recording confirms, stores new file, stays unprocessed until start again
- Upload/start disabled with **409** while `TRANSCRIBING` or `PROCESSING`
