...
} from '../../../../src/modules/agents/decision/services/decision.service';

describe('decision agent service', () => {
  ...
  it('short-circuits empty transcript without LLM call', async () => {
    const message = buildDecisionMessage(
      ...
      workspaceId,
      { meetingId, correlationId: 'empty-decision' },
    );

    const result = await decisionAgent.execute(message);
